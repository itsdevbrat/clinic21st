...
const UserForm = props => {
    ...
    const updateVerticals = (vertical, add) => {
        if (add) {
            ...
            verticalsSelected.filter(verticalSelected => {
                verticalSelected === vertical ? false : true
            })
        }
    }
    ...
    const verticalClicked = (idx) => {
        setVerticalsClicked(verticalsClicked.map((verticalClicked, index) => {
            if (index === idx && verticalClicked === true) {
                updateVerticals(verticals[idx], false)
                return !verticalClicked
            } else {
                updateVerticals(verticals[idx], true)
                return verticalClicked
            }
        }))
    }
