...
const UserForm = props => {
    ...
    const addVerticals = (vertical) => verticalsSelected.push(vertical)

    const updateVerticals = (vertical, add) => {
        if (add)
            verticalsSelected.push(vertical)
        else
            verticalsSelected.filter(verticalSelected => verticalSelected !== vertical)
    }
        ...
        setVerticalsClicked(verticalsClicked.map((verticalClicked, index) => {
            if (index === idx) {
                if (verticalClicked === true)
                    removeVertical(verticals[idx])
                else
                    addVertical(verticals[idx])
